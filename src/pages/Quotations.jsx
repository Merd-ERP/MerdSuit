...
function Quotations() {
  ...
  function resetForm() { setQuotation({ client: "", project: "", date: "" }); setMaterials([]); setLabour(""); setTransport(""); setDiscount(""); setEditingQuotation(null); }
  function editQuotation(savedQuotation) { setEditingQuotation(savedQuotation); setQuotation({ clientId: savedQuotation.clientId || "", client: savedQuotation.client || "", project: savedQuotation.project || "", date: savedQuotation.date || "" }); setMaterials(savedQuotation.materials || []); setLabour(savedQuotation.labour || ""); setTransport(savedQuotation.transport || ""); setDiscount(savedQuotation.discount || ""); window.scrollTo({ top: 0, behavior: "smooth" }); }
  return <MainLayout><PageHeader title="Quotations" subtitle="Create clear estimates for your electrical work." /><QuotationHeader /><QuotationStats /><Card>{editingQuotation && <div className="mb-6 rounded-xl bg-blue-50 p-4 text-sm font-medium text-blue-800">Editing draft {editingQuotation.quotationNumber}</div>}<QuotationForm quotation={quotation} setQuotation={setQuotation} clients={clients} projects={projects} /><QuotationItems materials={materials} addMaterial={addMaterial} updateMaterial={updateMaterial} deleteMaterial={deleteMaterial} /><div className="mt-8 grid gap-4 md:grid-cols-3"><label className="text-sm font-medium text-slate-700">Labour<input type="number" placeholder="Enter labour cost" value={labour} onChange={(event) => setLabour(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 p-3" /></label><label className="text-sm font-medium text-slate-700">Transport<input type="number" placeholder="Enter transport cost" value={transport} onChange={(event) => setTransport(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 p-3" /></label><label className="text-sm font-medium text-slate-700">Discount<input type="number" placeholder="Enter discount" value={discount} onChange={(event) => setDiscount(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-300 p-3" /></label></div><QuotationSummary materialTotal={materialTotal} labour={numericLabour} transport={numericTransport} discount={numericDiscount} finalTotal={finalTotal} /><QuotationQuickActions quotation={quotation} materials={materials} labour={numericLabour} transport={numericTransport} discount={numericDiscount} finalTotal={finalTotal} resetForm={resetForm} editingQuotation={editingQuotation} /></Card><Card className="mt-6"><QuotationTable onEdit={editQuotation} /></Card></MainLayout>;
}
