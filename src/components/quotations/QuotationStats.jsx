...
function QuotationStats() {
  ...
  const finalizedQuotations = quotations.filter((quotation) => quotation.status !== "Draft");
  const accepted = finalizedQuotations.filter((quotation) => quotation.status === "Accepted").length;
